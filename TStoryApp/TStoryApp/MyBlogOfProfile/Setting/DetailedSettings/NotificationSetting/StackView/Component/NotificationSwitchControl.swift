...
protocol NotificationSwitchControlDelegate: AnyObject {
...
class NotificationSwitchControl: UIControl {
    ...
    weak var delegate: NotificationSwitchControlDelegate?
    ...
    override func beginTracking(_ touch: UITouch, with event: UIEvent?) -> Bool {//UIControl touch 이벤트를 관리하는 메서드
        ...
        self.animate()
        delegate?.switchIsOnSetUI(index: self.tag, switchIsOn: self.switchIsOn)
        return true
    }
